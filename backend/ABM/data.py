...
def data_processing(market,household,query):
    ...
    # This is for future releases when users can select the best queries.
    # for i in query:
    #     a = i.replace("\\","")
    #     q   = ast.literal_eval(a[1:-1])

    # Changing this for 2023-06 release. We are selecting the top ranked answer for this release.
    q = ast.literal_eval(query[1:-1])
    ...
    # For future releases when users can select the best queries.
    # for i in q:
    ...
    # Changing this for 2023-06 release. We are selecting the top ranked answer for this release.
    if q["type"]=="modify" or q["type"]=="convert" or q["type"]=="add" or q["type"]== "remove":
        a_r_m_c_commands.append(q)
    elif q["type"]=="scope":
        scope_commands.append(q)
    elif q["type"]=="report":
        report_commands.append(q)
    command=None
    qty=None
    obj=None
    location = None
